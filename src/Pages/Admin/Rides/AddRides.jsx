...
const AddRides = () => {
  ...
  const routeOptions =
    selectionData?.data?.routes?.map((r) => ({ value: r.id, label: r.name })) ||
    [];
  const busOptions =
    selectionData?.data?.buses?.map((b) => ({
      value: b.id,
      label: `${b.busNumber} - ${b.plateNumber}`,
    })) || [];
  const driverOptions =
    selectionData?.data?.drivers?.map((d) => ({
      value: d.id,
      label: d.name,
    })) || [];
  const codriverOptions =
    selectionData?.data?.codrivers?.map((c) => ({
      value: c.id,
      label: c.name,
    })) || [];
...
},
{
  ...
  label: "End Date",
  type: "datetwo",
  // يظهر فقط لو النوع limited
  hidden: (formData) =>
    formData.frequency !== "repeat" || formData.repeatType !== "limited",
},
...

      {
        ...
        render: ({ formData }) => {
          ...
          const currentRoute = selectionData?.data?.routes?.find(
            (r) => String(r.id) === String(selectedRouteId)
          );

          const pickupOptions =
            currentRoute?.pickupPoints?.map((pp) => ({
...
const handleSave = async (formData) => {
  ...
  if (selectedStudents.length === 0)
    return toast.error("Please add at least one student");
            ...
            {selectedStudents.map((student) => (
              ...
              >
                ...
                <span className="font-medium">
                  {student.studentName}-{student.pickupTime}-
