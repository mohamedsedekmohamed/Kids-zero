...
const EditAdmins = () => {
 ...
 const parentsOptions =
  ...
  // 🔹 Get admin by id
  const { data, loading: loadingGet } = useGet(`/api/superadmin/subadmins/${id}`);

  // 🔹 Put admin
  const { putData, loading } = usePut(`/api/superadmin/subadmins/${id}`);

  const originalData = data?.data?.subAdmin;
  ...
  return (
    <AddPage
      ...
      initialData={{
        ...originalData,
        roleId: originalData?.roleDetails?.id
      }}
      onSave={handleSave}
      onCancel={() => navigate(-1)}
      loading={loading}
