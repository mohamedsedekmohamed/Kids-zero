...
const EditRoles = () => {
  ...
  const { data: selectionData, error: selectionError } = useGet("/api/superadmin/roles/permissions");
  const { data, loading: loadingGet } = useGet(`/api/superadmin/roles/${id}`);
  const { putData, loading: loadingPut } = usePut(`/api/superadmin/roles/${id}`);
