 ...
 <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
  ...
  <div className="relative w-full md:w-80">
    ...
    <input
      type="text"
      placeholder="Search..."
      className="w-full p-2 pl-10 border border-border rounded-lg bg-card text-foreground
                 focus:ring-2 focus:ring-one outline-none transition-all"
      value={searchTerm}
      onChange={(e) => {
  ...
  <button
    onClick={onAddClick}
    className="w-full md:w-auto bg-one hover:opacity-90 text-white
               px-6 py-2 rounded-lg font-bold transition-all shadow-md
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-muted/50 text-muted-foreground border-b border-border">
                ...
                {/* عمود العمليات يظهر فقط إذا مررنا renderActions */}
  ...
  <TruncatedText text={String(row[col.key] || "")} />
                    ...
                    {/* Render custom actions from parent */}
                    {renderActions && (
